...
export const add = (rootDirectory: string, objectStore: ObjectStore, indexStore: IndexStore) =>
  ...
  const addFile = async (path: string): Promise<void> => {
    ...
    const indexEntry = await createIndexEntry(rootDirectory, relativePath, blobObject.id);
